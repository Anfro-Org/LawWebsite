...
**Stats side lighting**: two candlelight washes (`.stats-glow.sg-l/.sg-r`, same color stops as `.rv-glow`, `mix-blend-mode:screen`, light theme: multiply at .55) flank the statistics strip and reach ~halfway down the About section (`top:-30vh; height:calc(100% + 150vh)` on a `position:relative` `.stats`; vertically-stretched ellipse `56% 60%` so it reads as a column of light, not a circle). Nothing clips them, so the radial falloff blends on all sides — no masks needed.
